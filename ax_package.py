...
def ax_forecast(df, lag, Tin, C_dict, d_dict):
    ...
    dfaug_coef: list
        m-k length list, each element of which is a (Tin+1)*(k+lag*m) dataframe
    ...
    # check1: make sure df is dataframe
    df = pd.DataFrame(df)
    
    # check2: make sure df elements are numeric
    ...
    u_col = df.columns[df.isna().any()].tolist()
    k_col = df.columns[~df.isna().any()].tolist()
    correct_order = u_col + k_col
    ...
    for i in C_dict:
        ...
        # if there is no free variables, the forecat can be solved by the constraints without 1st step
        if Ui.shape[0] == Ui.shape[1]: # if Ui is square, Ui is invertible from the previous step
...
def step1(df0, lag, Tin):
    ...
    for t in range(T_aug-Tin,T_aug+1): # forecast of T-Tin to T is for the weight matrix in the 2nd step
        ...
        fit_pca = PCA(n_components = 0.95).fit(X_data) # choose #components that generates 95% variance
        X_data_reduced = fit_pca.transform(X_data)
        X_pred_reduced = fit_pca.transform(X_pred)
        
        for ui in list(range(u)):
            print('forecasting time ',df0.index[t],' and variable ',df0.columns[ui])
            ...
            if np.isclose(np.std(y_data),0):
                ...
                fit_elasticnet = ElasticNetCV(cv=tscv, max_iter=100000, fit_intercept = False).fit(X_data, y_data)
    ...
    for t in range(-h+1, 0):
        ...
        df0_h = df0aug_h.loc[:,df0.columns]
